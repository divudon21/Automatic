package com.vibeflow.music.download
...
import com.vibeflow.music.extractor.YouTubeStreamResolver
...
suspend fun downloadTrack(
    ...
    val streamUrl: String = when (track.source) {
        ...
        MusicSource.AUDIOMACK -> {
            // Deezer: direct MP3 URL stored in videoId field
            val url = track.videoId
            Log.d(TAG, "Deezer URL: ${url.take(80)}")
            url
        }
    }
    ...
    val qualityTag = when (track.source) {
        ...
        MusicSource.AUDIOMACK -> "_preview"
    }
    ...
    try {
        ...
        withContext(Dispatchers.Main) {
            val qualStr = when (track.source) {
                ...
                MusicSource.AUDIOMACK -> "MP3 Preview"
            }
            Toast.makeText(
                context,
