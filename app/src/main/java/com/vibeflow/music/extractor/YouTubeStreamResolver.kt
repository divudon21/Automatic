 ...
 *  1. YouTube InnerTube ANDROID_MUSIC client  — direct URLs, no cipher
 *  2. YouTube InnerTube WEB client            — fallback
 *  3. Piped public instances (x5)             — community proxy fallback
 *
...
object YouTubeStreamResolver {
    ...
    suspend fun resolveWithQuality(videoId: String, quality: com.vibeflow.music.data.models.AudioQuality): String? =
        resolve(videoId)

    suspend fun resolve(videoId: String): String? {
        Log.d(TAG, "Resolving: $videoId")

        // 1. InnerTube Android Music client
